...
df = pd.read_csv(r'C:\Users\HP\Downloads\2024-09-18-data_export.csv')

#Check first 5 rows
print(df.head())

# Check for duplicate rows with the same value as the header
header_values = df.columns

# Remove any duplicate rows that are identical to the header
df = df[~df.eq(header_values).all(axis=1)]

# Remove value which couldn't be converted
